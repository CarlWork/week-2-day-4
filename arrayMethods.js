...
// CODE HERE
const postTaxPrices = prices.map((total) => total * 1.07);
...
// CODE HERE
const totalPopulation = populations.reduce((acc, curr) => {return acc + curr})
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(object => object.CP > 200)
...
const orderTotals = orders.map((order => order.price * (order.tax + 1)))
...
const bobsTotal = purchases.filter((purchase) => purchase.owner === "Bob").reduce((total, obj) => {return total + obj.price}, 0)
